...
const DADOS = {
  ...
  dimensoes: {
    ...
    */
    alturaAnoDesktop: 330,
    alturaAnoTabletHorizontal: 280,
    alturaAnoTelemovelHorizontal: 200,
...
/* ============================================================
   NUVENS — pequenas e distribuídas aleatoriamente
   ============================================================ */
...
.nuvem-pequena {
  ...
  object-fit: contain;
  opacity: .88;
  filter: drop-shadow(0 2px 1px rgba(80,130,160,.08));
}
...
function criarNuvens() {
  ...
  for (let i = 0; i < cfg.quantidade; i++) {
    ...
    do {
      left = cfg.margemHorizontal + Math.random() * (100 - cfg.margemHorizontal * 2);
      top = cfg.topoMin + Math.random() * (cfg.topoMax - cfg.topoMin);
      tentativas++;
    } while (usadas.some(p => Math.abs(p.left - left) < 18 && Math.abs(p.top - top) < 42) && tentativas < 30);

    usadas.push({ left, top });

    const img = document.createElement("img");
    const largura = cfg.larguraMin + Math.random() * (cfg.larguraMax - cfg.larguraMin);
    const altura = cfg.alturaMin + Math.random() * (cfg.alturaMax - cfg.alturaMin);
    ...
    img.style.transform = `translate(-50%, -50%) rotate(${(Math.random() * 6 - 3).toFixed(2)}deg)`;
